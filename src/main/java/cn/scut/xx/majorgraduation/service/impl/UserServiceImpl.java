...
public class UserServiceImpl extends ServiceImpl<UserMapper, UserPO> implements IUserService {
    ...
    public void addRole(UserRoleAddReqDTO userRoleAddReqDTO) {
        RolePO role = roleMapper.selectById(userRoleAddReqDTO.getRoleId());
        UserPO user = baseMapper.selectById(userRoleAddReqDTO.getUserId());
        if (role == null || user == null) {
            throw new ClientException("用户或角色不存在！");
        }
